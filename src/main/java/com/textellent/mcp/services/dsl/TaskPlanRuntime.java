 ...
 */
public final class TaskPlanRuntime {

    private static final Logger logger = LoggerFactory.getLogger(TaskPlanRuntime.class);
    ...
    public void execute(
        ...
        logger.info("TaskPlanRuntime: starting task execution");
        List<String> validationErrors = validator.validateTask(task);
        if (!validationErrors.isEmpty()) {
            logger.warn("TaskPlanRuntime: task validation failed, aborting execution");
        ...
        for (int p = 0; p < phases.size(); p++) {
            ...
            if (dependencyFailed) {
                logger.info("TaskPlanRuntime: phase {} skipped (dependency failed)", phaseId);
            ...
            logger.info("TaskPlanRuntime: executing phase {}/{} id={}", p + 1, phases.size(), phaseId);
        ...
        logger.info("TaskPlanRuntime: task finished, status={} varsKeys={}", overallResult.get("status"), vars.keySet());
    }
    ...
    private Map<String, Object> executePhase(
        ...
        for (int s = 0; s < steps.size(); s++) {
            ...
            logger.debug("TaskPlanRuntime: phase {} step {}/{} id={} type={}", phaseId, s + 1, steps.size(), stepId, type);
            ...
            try {
                switch (type != null ? type : "") {
                    ...
                    case "batchLoop":
                        ...
                        break;
                    case "wait":
                        logger.info("TaskPlanRuntime: ignoring wait step {} (MVP)", stepId);
                        break;
                    case "compute":
                        executeComputeStep(step, vars);
                        break;
                    case "checkpoint":
                        logger.info("TaskPlanRuntime: checkpoint step {} not yet active; continuing", stepId);
                        break;
                    default:
                        logger.warn("TaskPlanRuntime: unknown step type '{}' for id={}, skipping", type, stepId);
                }
            } catch (Exception e) {
                logger.error("TaskPlanRuntime: step id={} type={} failed: {}", stepId, type, e.getMessage(), e);
    ...
    private void executeToolCallStep(
        ...
        if (capture != null && !capture.trim().isEmpty()) {
            ...
            logger.debug("TaskPlanRuntime: captured into vars.{}", capture);
        }
    }
    ...
    private void executeBatchLoopStep(
        ...
        while (i < allItems.size() && batchIndex < maxBatches) {
            ...
            while (!batchSucceeded && attempts <= maxRetries) {
                ...
                try {
                    ...
                    if (capture != null && !capture.trim().isEmpty()) {
                        vars.put(capture, parsed);
                        logger.debug("TaskPlanRuntime: batchLoop captured into vars.{}", capture);
                    }
        ...
        logger.info("TaskPlanRuntime: batchLoop finished, {} batch(es), {} item(s) total", batchIndex, allItems.size());
    }
