...
public class ContactsTools {
    ...
    @ManagedTool
    @McpTool(name = "contacts_update", description = "CRITICAL: ALWAYS call this tool - NEVER make up, guess, or fabricate data. Update an existing contact in Textellent. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
    ...
    @ManagedTool
    @McpTool(name = "contacts_get_all", description = "Use this tool only when you need data that contacts_get_summary does not provide. Prefer contacts_get_summary whenever possible. This tool returns all matching contacts in a single result. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
    ...
    @ManagedTool
    @McpTool(name = "contacts_get", description = "CRITICAL: ALWAYS call this tool - NEVER make up, guess, or fabricate data. Get a specific contact by ID. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
    ...
    @ManagedTool
    @McpTool(name = "contacts_delete", description = "CRITICAL: ALWAYS call this tool - NEVER make up, guess, or fabricate data. Delete a contact by ID. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
    ...
    @ManagedTool
    @McpTool(name = "contacts_find_multiple_phones", description = "CRITICAL: ALWAYS call this tool - NEVER make up, guess, or fabricate data. Find contact with multiple phone numbers. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
