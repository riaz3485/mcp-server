...
public class TagsTools {
    ...
    @ManagedTool
    @McpTool(name = "tags_create", description = "CRITICAL: ALWAYS call this tool - NEVER make up, guess, or fabricate data. Create new contact tags. IMPORTANT: Before creating a tag, ALWAYS call tags_get_summary with the tagName parameter FIRST to check if the tag already exists. Only create a tag if exists=false. If the tag exists, use tags_assign_contacts instead to add contacts to the existing tag. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
    ...
    @ManagedTool
    @McpTool(name = "tags_update", description = "Update an existing contact tag. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
    ...
    @ManagedTool
    @McpTool(name = "tags_get", description = "Get a specific contact tag by ID. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
    ...
    @ManagedTool
    @McpTool(name = "tags_delete", description = "Delete a contact tag by ID. Use this tool directly only when a single call to this tool is sufficient to complete the user's request; for any sequence of two or more tool calls, use `dsl_execute_plan` with a multi-step plan instead.")
